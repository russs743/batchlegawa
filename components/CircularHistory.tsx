...
export default function CircularHistory() {
  ...
  const ringTranslateProgress = useTransform(
    smoothProgress,
    [0.5, 0.75, 1],
    [0, 1, 1],
  );
  const ringTranslateX = useTransform(
    ringTranslateProgress,
    (p) => p * targetX,
  );
  ...
  const globalRotateZ = useTransform(
    () => scrollRotateZ.get() + navRotationOffset.get(),
  );
      ...
      <div className="sticky top-0 w-full h-screen overflow-hidden bg-theme-bg">
        ...
        >
          <p className="font-sans text-sm md:text-base text-theme-text/80 dark:text-white/80 leading-relaxed font-medium drop-shadow-sm">
            Thirteen paths forged into one history. Explore the journey of our
            high-growth chapter.
          </p>
        </motion.div>
