...
export default function RosterClient({ initialComments }: { initialComments: any[] }) {
  ...
  const memberComments = initialComments.filter(c => c.target.toLowerCase() === activeMember.name.toLowerCase());
            ...
            {(!isMobile || isMobileStatsOpen) && (activeMember as any).stats && (
              ...
              >
                ...
                <button 
                  ...
                  className="mt-2 pointer-events-auto w-full py-1.5 md:py-3 px-3 md:px-4 border border-white/20 bg-white/5 hover:bg-white hover:text-black text-white transition-colors duration-300 font-sans text-[0.55rem] md:text-sm tracking-[0.2em] uppercase rounded-full flex items-center justify-center gap-2 group/msg"
                >
                  <span>Read Messages</span>
                  <span className="bg-theme-accent text-white px-2 py-0.5 rounded-full text-[10px]">{memberComments.length}</span>
                </button>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      <AnimatePresence>
        {isMessageModalOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-200 flex items-center justify-center p-4 md:p-10 bg-black/80 backdrop-blur-sm pointer-events-auto"
          ...
          >
            <motion.div
              ...
              className="relative w-full max-w-5xl max-h-[85vh] bg-[#2a2a2a] rounded-3xl overflow-hidden border-8 border-[#4a3b32] shadow-2xl flex flex-col"
              style={{
            ...
            >
              <div className="absolute top-4 right-4 z-50">
                <button onClick={() => setIsMessageModalOpen(false)} className="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors">
